...
class SceneSequentialBatchSampler(torch.utils.data.Sampler):
    def __init__(self, scene_tokens, batch_size, drop_last=False):
        if batch_size <= 0:
        ...
        for idx, token in enumerate(scene_tokens):
            scene_to_indices.setdefault(token, []).append(idx)

        for token in sorted(scene_to_indices.keys()):
            indices = scene_to_indices[token]
...
def get_dataset(config, type):
    ...
    if type == 'train':
        use_sequential_memory = getattr(config, 'memory_train_mode', 'off') == 'sequential_epoch'
        if use_sequential_memory:
            scene_batch_size = int(getattr(config, 'batch_size', 1))
            scene_tokens = data.scene_tokens
            batch_sampler = SceneSequentialBatchSampler(
...
class REDS_Dataset:
    def __init__(self, config, type):
        ...
        self.return_scene_token_train = (self.type == 'train' and getattr(self.config, 'memory_train_mode', 'off') == 'sequential_epoch')
        self.return_scene_token_val = (self.type == 'val' and getattr(self.config, 'memory_eval_mode', 'off') == 'sequential')
        self.return_scene_token_test = (self.type == 'test' and getattr(self.config, 'memory_eval_mode', 'off') == 'sequential')
        ...
        self.seq_path = self.get_seq_path(bath_path) # [[0,1,2], [1,2,3], [2,3,4]..., [98,99,100]]
        self.scene_tokens = [self.get_scene_token(paths) for paths in self.seq_path]
    ...
    def __getitem__(self, idx):
        ...
        if self.type == 'train':
            ...
            if self.return_scene_token_train:
                scene_token = self.get_scene_token(lr_blur_path)
                return self.np2tensor(lr_blur_seq), self.np2tensor(hr_sharp_seq), self.np2tensor(lr_sharp_seq), self.flow2tensor(flow), scene_token
            return self.np2tensor(lr_blur_seq), self.np2tensor(hr_sharp_seq), self.np2tensor(lr_sharp_seq), self.flow2tensor(flow)

        if self.type == 'val':
            if self.return_scene_token_val:
                scene_token = self.get_scene_token(lr_blur_path)
                return self.np2tensor(lr_blur_seq), self.np2tensor(hr_sharp_seq), self.np2tensor(lr_sharp_seq), self.flow2tensor(flow), scene_token
            return self.np2tensor(lr_blur_seq), self.np2tensor(hr_sharp_seq), self.np2tensor(lr_sharp_seq), self.flow2tensor(flow)
        ...
        if self.type == 'test':
            ...
            if self.return_scene_token_test:
                scene_token = self.get_scene_token(lr_blur_path)
                return self.np2tensor(lr_blur_seq), filename, scene_token
    ...
    def get_seq_path(self, bath_path):
        ...
        for dir in dir_list:
            ...
            start = (self.num_seq - 1) // 2   # num_seq = 3
            end = len(frame_list) - (self.num_seq - 1) // 2
            for i in range(start, end):  # i = 中心帧的下标
                frame_seq = []
    ...
    def get_scene_token(self, lr_blur_path):
        center_frame_path = lr_blur_path[self.num_seq // 2]
        return os.path.basename(os.path.dirname(center_frame_path))
